...
You can also withdraw to another wallet you own on the Hyperliquid L1 using the `/withdraw` command. Once withdrawn, you can go to [https://app.hyperliquid.xyz/portfolio](https://app.hyperliquid.xyz/portfolio) and connect with your wallet and withdraw USDC from Hyperliquid to Arbitrum. From there, you can 

**How can I connect my existing Hyperliquid account to PVP?**

Use the `/accounts` command, and then you can attach your existing Hyperliquid account by pressing “Add New Hyperliquid Account”. You’ll then be prompted to enter your wallet address and to generate an API key, which we’ll use to authenticate your account. You will not

**What fees are collected on trades by PVP?**

There is a 0.05 fee on futures trades, and a 0.5% fee on spot trades. There is no fee on bridging or liquidations.
